...
<script>
    ...
    $cardContainer.addEventListener('click', e => {
        ...
        if (e.target.matches('.card-btn-group *')) {
            ...
            $confirmDelete.onclick = e => {
                // 삭제 요청을 서버에 보내야 한다.
                location.href = deleteLocation; // "/board/delete?bno=${b.boardNo}"
  ...
  function fixSearchOption() {
    ...
    // 셀렉트박스 내에 있는 option 태그들 전부 가져오기
    const $options = [...$select.children];
    $options.forEach($opt => {
        if($opt.value === '${s.type}') { // s는 서버의 컨트롤러에서 받은 Search 객체 page
            // option태그에 selected를 주면 그 option이 고정됨.
