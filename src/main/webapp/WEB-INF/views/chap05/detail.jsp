...
<script>
    ...
    function makePageButtonClickHandler() {
        ...
        $pageUl.onclick = e => {
            // 이벤트 타겟이 a태그가 아니면 href 속성을 못가져올 수 있으니 타겟 제한하기
    ...
    function makeReplyRemoveClickHandler() {
        ...
        $replyData.onclick = e => {
            ...
            // 수정이든 삭제든 댓글 번호가 필요.
            // 이벤트가 발생된 곳(수정, 삭제버튼)에서 가장 가까운 #replyContent에 붙은 댓글번호
            const rno = e.target.closest('#replyContent').dataset.replyid;

            if (e.target.matches('#replyDelBtn')) { // 삭제 로직 진행
                /*
                    confirm함수 이용해서 한번 더 물어보기
                    URL: /api/v1/replies/rno: DELETE, 전달되는 JSON은 없음
                    headers와 body는 작성할 필요 x
            ...
            } else if (e.target.matches('#replyModBtn')) { // 수정 모드로 진입(모달)
            ...
            // 저 두개 이외의 이벤트가 발생하면 무시. return 쓸 필요 없음.
        }
        ...
        $modBtn.addEventListener('click', e => {
            ...
            fetch(URL, requestInfo)
                .then(res => {
                    if (res.status === 200) {
                        alert('댓글이 수정되었습니다.');
                        document.getElementById('modal-close').click(); // modal창 닫기
                        return res.text();
                    } else {
                        alert('수정값에 문제가 있습니다. 내용을 확인하세요.');
                        document.getElementById('modReplyText').value = ''; // reply text 영역
                        return;
                    }
                })
                .then(data => {
