...
//        dto.setPassword(encodedPw);
    ...
    public LoginResult authenticate(LoginRequestDTO dto,
        ...
        if (dto.isAutoLogin()) {
            ...
            // 2. 쿠키 설정 - 사용 경로, 수명
            int limitTime = 60 * 60 * 24 * 90; // 자동 로그인 유지 시간 (90일)
    ...
    public void autoLoginClear(HttpServletRequest request, HttpServletResponse response) {
        ...
        // 2. 쿠키를 삭제한다. -> 쿠키의 수명을 0초로 설정하여 다시 클라이언트에 전송 -> 자동 소멸
        if (c != null) {
            c.setMaxAge(0);
            c.setPath("/");
            response.addCookie(c);

            // 3. 데이터베이스에서도 세션아이디와 만료시간을 제거
    ...
    public void kakaoLogout(LoginUserResponseDTO dto, HttpSession session) {
        ...
        // DB의 access_token의 값을 update를 해서 null로 만들어 주시면 됩니다.
